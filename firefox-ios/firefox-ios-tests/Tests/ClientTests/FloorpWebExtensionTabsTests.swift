...
final class FloorpWebExtensionTabsTests: XCTestCase {
    ...
    func testProfileTabsHostUsesLiveWindowManagersAndMessageBridge() async throws {
        ...
        let originalWebView = MockTabWebView(frame: .zero, configuration: .init(), windowUUID: manager.windowUUID)
        ...
        let liveCreatedWebView = MockTabWebView(frame: .zero, configuration: .init(), windowUUID: manager.windowUUID)
...
private final class LiveTabsManagerStub: MockTabManager {
    ...
    override func addTab(_ request: URLRequest?, afterTab: Tab?, zombie: Bool, isPrivate: Bool) -> Tab {
        ...
        let webView = MockTabWebView(frame: .zero, configuration: .init(), windowUUID: windowUUID)
        tab.webView = webView
        if let request {
