...
Status: **P1–P4 local implementation checks are recorded below. The exact
17-package Dark Reader MV3 input was signed and release-verified from clean
`main` commit `b133ab51243ae829ed24ebdd52f3fa848204ea91`; its schema 2
approval record is canonical and candidate-bound. Normal public-output PR/CI/
`main` integration, protected approval-digest registration, an immutable tag,
and the candidate workflow are still required before any App Store Connect
mutation. Existing TestFlight builds are not source-bound to this candidate, and
no physical-device result exists yet.**
...
| Curated catalog construction / inspection | pass — 57 Python tests | All 17 constrained packages were normalized as FWEA1; the project-resource check proves that only `Artifacts/` is placed in the app target, never source packages or review records. This is not a signed catalog by itself. |
| Curated artifact functional harness | pass — 17 / 17 | The local DOM/browser harness decodes every checked-in immutable FWEA1, verifies its catalog-record artifact/inventory/manifest digests and exact byte count, then exercises each constrained artifact's declared content-script, DNR, popup/options, or alarm behavior without remote code or remote lists. This proves the smoke paths use the bytes that would be bundled, not the review source tree. |
...
| Historical pre-Dark Reader catalog build/ingest and signing-handoff regression | pass — 34 / 34; deterministic 16 / 16 | This row records the prior 16-package contract only. The current 17-package input is independently covered by the checks below. |
...
| TestFlight release-workflow contract | pass — local contract tests | 任意 `catalog_release` switch を通常 TestFlight workflow に足す方式は、switch を外すと verifier を通らず mutation へ到達できるため撤回した。通常 workflow と候補 workflow を分離し、candidate は protected annotated `floorp-catalog-*` tag、`main` 到達確認、root-key digest と fixed 17-package verifier の Apple credential 前実行、tag-only `sourceBranchOrTag`、強制 `--wait` と completed-run source SHA readback を要求する。candidate workflow は external group/Beta App Review mutation を含まない。通常の `Build and unit test` には鍵を要しない catalog build/documentation/provenance/ingest/resource/workflow contract と 17-artifact functional harness を追加した。実際の protected secret、Xcode Cloud invocation、または App Store Connect mutation は行っていない。 |
...
| Historical pre-Dark Reader exact-main non-secret contract rerun | pass — 90 Python tests; functional 16 / 16 | 2026-08-28 に `ab6235fd7c6694aa97217d1626411ef0af0a3796` の clean checkout で実行した、旧 16-package candidate の証跡。現在の 17-package candidate には流用しない。 |
...
| Dark Reader native MV3 regression XCTest | pass — 49 / 49 | storage/i18n、MV3 API host、runtime message の current-source suites を実行し、0 failures を確認した。17-package signed public output の app-bundle audit と physical-device/P5 evidence は tag-bound candidate build 後に別途記録する。 |
| Historical signed-candidate P1–P4 simulator XCTest | pass — 149 / 149 | iPhone 17 / iOS 26.2 の新規 build (`Fennec_Testing`, FloorpCI)で package-store、content-script/private isolation、DNR、alarms/action、storage/i18n、page host、API host suites を実行し、149 passed / 0 failed / 0 skipped。これは prior signed candidate の証跡であり、current 17-package app-bundle candidate の代替ではない。 |
| Historical signed-candidate app-bundle resource audit | pass — 16 artifacts + 2 signed resources | 同じ arm64 simulator build の prior 16-package candidate の証跡。current 17-package signed public output の app-bundle audit は tag-bound candidate build 後に記録する。 |
...
| Historical arm64 app-bundle resource audit | pass — 16 artifacts / fail closed | UI fail-closed 修正後の iPhone 17 / iOS 26.2 `Fennec_Testing` build product を再監査した prior unsigned candidate の記録。current 17-package managed signer output の app-bundle audit は tag-bound candidate build 後に記録する。 |
...
| candidate adoption and P0 authority | 2026-08-28: the sole Floorp iOS maintainer approved the fixed, app-bundled 17 extensions, 1Password managed-key operation, signed revocation, private-mode/data-retention policy, and closed-install safety boundary. The policy receipt is `floorp-ios-webextensions-curated-catalog-p0-policy-approval.json`; the exact signed candidate is bound by its separate canonical schema 2 record. Managed signing and Beta App Review submission remain subject to normal PR/CI/`main` integration and all technical gates. |
...
| candidate tester / reviewer materials | prepared, not submitted | `firefox-ios/TestFlight/WhatToTest.en-US.txt` と `WhatToTest.ja-JP.txt` を、固定17本・同梱のみ・fail-closed を前提とする候補用手順に更新した。`floorp-ios-webextensions-internal-catalog-beta-app-review-draft.md` は exact schema 2 P0 record、source-bound `main` build、live App Store Connect reviewer contact を得た後だけ使う未提出ドラフトである。既存 App Store Connect の Beta App Review details は編集していない。 |
