...
34/34, but the candidate has no managed-signer-produced catalog or root public key yet;
it therefore fails closed and must not be represented as this catalog release. An
existing TestFlight build is not source-bound to this unmerged change. No author-approved
signed artifact or physical-device result exists yet.**
...
| Curated artifact functional harness | pass — 16 / 16 | The local DOM/browser harness decodes every checked-in immutable FWEA1, verifies its catalog-record artifact/inventory/manifest digests and exact byte count, then exercises each constrained artifact's declared content-script, DNR, popup/options, or alarm behavior without remote code or remote lists. This proves the smoke paths use the bytes that would be bundled, not the review source tree. |
...
| Current P2–P4 simulator XCTest | pass — 76 / 76 | UI fail-closed と managed signing handoff の修正後、iPhone 17 / iOS 26.2 の `Fennec_Testing` で同じ 6 suites を再実行。content script の host grant/失効/all-frame/document-start、通常/private 分離、static-block DNR と非対応 action 拒否、popup/options/action 無しの安全な空状態、storage/alarms/native consent を確認。Result bundle: `Test-Fennec-2026.08.27_13-42-01-+0900.xcresult`。 |
| Current curated catalog build/ingest and signing-handoff regression | pass — 34 / 34; deterministic 16 / 16 | 16 artifact の deterministic FWEA1 構築、project resource の shipping 範囲、CRX/ZIP の quarantine、remote executable/dynamic code rejection、署名 catalog canonicalization、managed signer handoff、release-only signed-catalog verifier を再確認。新 verifier は synthetic root/leaf で canonical signature、protected root digest mismatch、catalog tamper、artifact tamper、version mismatch、expiry、future-dated revocation、evidence overwrite を fail closed で拒否する。3 Floorp-managed source は full commit SHA、13 第三者 source は immutable revision に固定する。Very Good AdBlock の archive/member/rule mapping は synthetic regression で verifier 契約を確認しており、候補用の実 archive 再照合と candidate-bound provenance evidence は signer invocation の必須入力として未完了である。archive reader は compressed byte、member count、declared expanded byte、target member size、duplicate target を fail closed で制限する。handoff test は同一 Git checkout、検証済み byte snapshot、private key 読込前の final clean recheck、public output と external evidence の原子的非上書きを固定する。現在の unsigned `catalog-input.json` SHA-256 は `7230ee7b84cc35d859ee92482cb240e2757fd1974bd06ac27d64b91499c7b39b`。managed private key や署名済み release output は生成していない。 |
| Actual unsigned-tree release-contract check | pass — fail closed | `verify_signed_curated_catalog_release.py` を current catalog root に対して実行し、`Artifacts/Signed/catalog.json` が存在しないため release candidate を拒否することを確認した。これは署名済み候補の成功証跡ではなく、unsigned artifacts が TestFlight workflow を通過しないことの証跡である。 |
| TestFlight release-workflow contract | pass — local contract tests | 任意 `catalog_release` switch を通常 TestFlight workflow に足す方式は、switch を外すと verifier を通らず mutation へ到達できるため撤回した。通常 workflow と候補 workflow を分離し、candidate は protected annotated `floorp-catalog-*` tag、`main` 到達確認、root-key digest と fixed 16-package verifier の Apple credential 前実行、tag-only `sourceBranchOrTag`、強制 `--wait` と completed-run source SHA readback を要求する。candidate workflow は external group/Beta App Review mutation を含まない。通常の `Build and unit test` には鍵を要しない catalog build/documentation/provenance/ingest/resource/workflow contract と 16-artifact functional harness を追加した。実際の protected secret、Xcode Cloud invocation、または App Store Connect mutation は行っていない。 |
| Current candidate-source control regression | pass — release 65 / 65; catalog 34 / 34; functional 16 / 16 | 2026-08-27: exact annotated SHA tag / current-main / canonical tag-kind guard、signed-catalog verifier-before-credentials、external submission separation、Xcode Cloud `sourceBranchOrTag` resolution を再実行した。新しい post-clone harness は actual `CI_GIT_REF`/`CI_TAG`/`CI_COMMIT`、manual start、workflow、bundle ID、checkout、fresh `origin/main` の完全一致だけを通し、tag/commit 不一致、main advance、automatic start を archive 前に拒否する。Bash syntax、workflow YAML、`git diff --check` も成功。これは local control evidence であり、Xcode Cloud/App Store Connect の実行証跡ではない。 |
| Current candidate-source security diff scan | pass — 0 findings | 2026-08-27 sealed scan `59a67bc9-776a-43c0-b227-6ac1f8477f29`、snapshot `codex-security-snapshot/v1:sha256:2252558baf24bc7f9d7e12e9d72afbaa467b14a425bafb1354555783d8fdf021`。candidate GitHub/Xcode source binding、reference kind/canonical namespace、credential ordering、external mutation separation、signed runtime/atomic lifecycle/profile/DNR boundaryを含む 25 changed surfaces をレビューした。Apple、Legal/Privacy、managed signer、GitHub/Xcode Cloud policy、物理実機、External TestFlight は source-scan 対象外の release gate であり、合格と解釈してはならない。 |
...
| Current arm64 app-bundle resource audit | pass — 16 artifacts / fail closed | UI fail-closed 修正後の iPhone 17 / iOS 26.2 `Fennec_Testing` build product を再監査。16 個の `Artifacts/*.fwea1`、`Artifacts/Signed/catalog.json` と `root-public-key.txt` は 0、ZIP/XPI/CRX、`catalog-input.json`、`catalog-sources.json` は 0 件だった。よって managed signer output がない現時点では catalog は導入不能のまま fail closed である。 |
...
| --- | --- |
| main merge SHA | 未記録 |
| protected candidate tag / tag policy evidence | 未記録 — required form is annotated `floorp-catalog-<40 lowercase commit SHA>` for the then-current exact `main` HEAD; policy must prohibit force-update/delete except approved release recovery. |
| release-candidate workspace / base | `codex/floorp-curated-extension-catalog` の local working tree（base feature commit `940118d86a3966932881cac5dd46c7b6071dfc00`、未push の追加差分あり）/ `origin/main` `c7c8490af409cc1ff99bec2f576cd8eea3ff739a`。通常 review、CI、正規 merge により exact `main` SHA が生じ、managed signer output をその SHA に束縛してから初めて External TestFlight source candidate になる。 |
...
| candidate tester / reviewer materials | prepared, not submitted | `firefox-ios/TestFlight/WhatToTest.en-US.txt` と `WhatToTest.ja-JP.txt` を、固定16本・同梱のみ・fail-closed を前提とする候補用手順に更新した。`floorp-ios-webextensions-internal-catalog-beta-app-review-draft.md` は P0 全承認・source-bound `main` build・実在する担当者情報を得た後だけ使う未提出ドラフトである。既存 App Store Connect の Beta App Review details は編集していない。 |
| workflow / Xcode Cloud run for this source | not started — requires the merged exact current main SHA, protected annotated `floorp-catalog-<40 lowercase commit SHA>` tag whose name/tag commit/checkout/current `origin/main` HEAD agree, managed signer output in `Artifacts/Signed/catalog.json` and `Artifacts/Signed/root-public-key.txt`, the candidate-only release-contract success, a pre-archive Cloud check of the actual tag/ref/commit/workflow/bundle/checkout/current-main identity, and a terminal Xcode Cloud `sourceCommit` match. 2026-08-27 の secret-name-only read では protected `floorp-testflight` environment に `FLOORP_CURATED_CATALOG_ROOT_PUBLIC_KEY_SHA256` が無く、current app bundle audit も signed files がないことを確認した。 |
