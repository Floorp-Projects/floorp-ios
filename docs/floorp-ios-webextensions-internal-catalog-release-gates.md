...
- 2026-08-28: GitHub Environment API の read-only metadata で、両方の curated environment に
  ...
  は読み取らず、署名済み root と照合するまで trust anchor は完了扱いにしない。両 environment は
  required reviewer protection をまだ持たないため、external mutation は引き続き blocked である。
...
| `RELEASE_TRUST_ANCHOR_MISSING` | partially verified / blocked | Security / Release | 両 curated environment に `FLOORP_CURATED_CATALOG_ROOT_PUBLIC_KEY_SHA256` secret 名が存在することは read-only metadata で確認済み。ただし value は読まず、approved root public key の raw-32-byte SHA-256、key ID/custody record、実署名済み root public key を三者照合するまでは完了にしない。source tree の `root-public-key.txt` 自身を信頼根拠にしてこの条件を満たしてはならない。 |
