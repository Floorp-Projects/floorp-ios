...
| Tabs query/create/update/reload/sendMessage | Supported subset | The profile- and private-mode-scoped host uses live app tabs, rejects stale document generations before delivery, supports active/current/all queries with URL/title redaction unless `tabs` or matching host access is granted, and permits only profile-scoped navigation, reload, creation, and isolated-world content messaging. |
| `storage.sync` | Supported subset | A separately persisted, profile-owned device-local namespace supports the standard key/value and change-event API. It never uses a cloud account, sync transport, or cross-profile storage; private storage is ephemeral. |
| `runtime.getManifest`, Chrome i18n, and `action.setIcon` | Supported subset | Package-derived manifest/localization data are bootstrapped synchronously for Chrome-compatible calls. Action icons must be reviewed package image resources; per-tab icons are unsupported. |
| `fontSettings` | Supported subset | The permission grants only the generic iOS font fallback used by compatible packages. The runtime does not expose native font enumeration or per-script font preference controls. |
