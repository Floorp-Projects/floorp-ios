...
Remote push notifications, the hosted summarizer, and Quick Answers also remain
disabled. Quick Answers has its own build policy because it creates a separate
Mozilla MLPA/App Attest client; a Nimbus enrollment cannot override that policy.
...
| App Store Connect app ID | `6796708699` (keep `FLOORP_APP_STORE_ID` empty until the public listing is live) |
...
7. Confirm that Push, Hosted Summarizer, and Quick Answers remain disabled
   unless Floorp has the required capability, service ownership, privacy
   disclosure, and production tests.
