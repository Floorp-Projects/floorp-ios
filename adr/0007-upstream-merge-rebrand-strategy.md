...
Accepted; amended 2026-08-01 to separate reviewed localization generation from code and asset branding.
...
Localized product-name values are intentionally excluded from broad replacement. They are generated by `scripts/l10n/floorp-l10n-overlay.mjs` from the reviewed policy and coverage under `floorp/l10n/`. The overlay changes only covered value spans, preserves protected Mozilla service names, rejects source drift, and can resolve only ordinary localization conflicts that it can prove are generated on both sides. A conflict outside that proof remains manual.
